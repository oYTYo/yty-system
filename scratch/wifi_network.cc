 ...
 * * 拓扑结构:
 * 12个 Wi-Fi 摄像头 (STA) -> 1个无线接入点 (AP) -> (有线P2P瓶颈链路) -> 1个服务器
 *
 * 主要特点:
 * 1. 物理层: 802.11n 5GHz, 80MHz 频宽, MIMO 2x2 (复用自 large-scale 配置).
 * 2. 摄像头配置: 包含6个H.264编码的摄像头和3个H.265编码的摄像头(混合模式).
...
int main(int argc, char* argv[])
{
    ...
    // --- 仿真核心参数 ---
    const uint32_t WIFI_CAM_TOTAL = 12; // 12个Wi-Fi摄像头
    ...
    WifiHelper wifi;
    wifi.SetStandard(WIFI_STANDARD_80211n); // 使用 802.11n (或者 WIFI_STANDARD_80211ac)
    ...
    p2pBackhaul.SetDeviceAttribute("DataRate", StringValue("50Mbps")); // 初始值，会被动态脚本覆盖
    ...
    serverHelper.SetAttribute("LogFile", StringValue("scratch/play_status_wifi.txt"));
